...
def new_data_array_by_mask_set_to_single_value(data_array, bool_mask_da, scalar):
    return xr.where(bool_mask_da, scalar, data_array)

def new_data_array_by_mask_add_criteria_scalar_fn(data_array, bool_mask_da, zero_criteria_code):
    return xr.where(bool_mask_da, data_array + 2**(zero_criteria_code - 1), data_array)

def new_data_array_by_mask_add_scalar(data_array, bool_mask_da, scalar):
        ...
        if prof_key in MITprof_ds:
            ...
            for zero_criteria_code in zero_criteria_codes:
                ...
                if zero_criteria_code == 3: #  missing T or S
                    ...
                    MITprof_ds[prof_key] = new_data_array_by_mask_set_to_single_value(MITprof_ds[prof_key], bool_mask_da, np.nan)
                    MITprof_ds[f'{prof_key}weight'] = new_data_array_by_mask_set_to_single_value(MITprof_ds[f'{prof_key}weight'], bool_mask_da, 0)
                    MITprof_ds[f'{prof_key}weight_code'] = new_data_array_by_mask_add_criteria_scalar_fn(MITprof_ds[f'{prof_key}weight_code'], bool_mask_da, zero_criteria_code)
                ...
                if zero_criteria_code == 7: # illegal dates/times
                    ...
                    for ii in np.arange(num_profs):
                        ...
                        #tmp = str(MITprof_ds['prof_YYYYMMDD'][ii])
                        y[ii]  = int(tmp[0:4])
                        m[ii]  = int(tmp[4:6])
                        d[ii]  = int(tmp[6:8])
                    ...
                    if prof_key == 'prof_S':
                        if len(MITprof_ds[prof_key].shape) > 1:
                            ...
                            for ii in range(len(var_dict[prof_key]['subsurface_min_depth_thresholds'])):
                                ...
                                threshold_flag_array = new_data_array_by_mask_set_to_single_value(threshold_flag_array, bool_mask_da_S_below_threshold_val_bad, 1)
                                threshold_flag_array = new_data_array_by_mask_set_to_single_value(threshold_flag_array, bool_mask_da_S_above_threshold_val_good, 0)
