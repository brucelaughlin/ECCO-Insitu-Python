...
def chunk_modify_in_place_using_depth_index_set_to_nan(arr, index, chunk_size):
    for ii in range(0, len(index), chunk_size):
        arr[:, index[ii:ii+chunk_size]] = np.nan

def chunk_modify_in_place_using_depth_index_set_to_zero(arr, index, chunk_size):
    for ii in range(0, len(index), chunk_size):
        arr[:, index[ii:ii+chunk_size]] = 0   

def chunk_modify_in_place_using_depth_index_add_scalar(arr, index, scalar, chunk_size):
    for ii in range(0, len(index), chunk_size):
        arr[:, index[ii:ii+chunk_size]] += scalar



def chunk_modify_in_place_using_profile_index_set_to_nan(arr, index, chunk_size):
    for ii in range(0, len(index), chunk_size):
        arr[index[ii:ii+chunk_size], :] = np.nan

def chunk_modify_in_place_using_profile_index_set_to_zero(arr, index, chunk_size):
    for ii in range(0, len(index), chunk_size):
        arr[index[ii:ii+chunk_size], :] = 0   

def chunk_modify_in_place_using_profile_index_add_scalar(arr, index, scalar, chunk_size):
    for ii in range(0, len(index), chunk_size):
        arr[index[ii:ii+chunk_size], :] += scalar
...
def count_profs_with_nonzero_weights(MITprofs):
    ...
    nonzero_T_ins = np.where(np.nansum(MITprofs['prof_Tweight'], axis = 1) > 0)[0]
    #nonzero_T_ins = np.where(np.sum(MITprofs['prof_Tweight'], axis = 1) > 0)[0]
    num_nonzero_T = len(nonzero_T_ins)
    zero_weight_T_ins = np.where(np.nansum(MITprofs['prof_Tweight'], axis= 1) == 0)[0]
...
def update_zero_weight_points_on_prepared_profiles(run_code, MITprofs):
    ...
    for zero_criteria_code_i in zero_criteria_code:
        ...
        if zero_criteria_code_i == 2: # nonzero prof T or S flag
            ...
            #MITprofs['prof_Tweight'][ins1] = 0    
            #zero_T_weight_reason[ins1] = zero_T_weight_reason[ins1] + 2**(zero_criteria_code_i - 1)
            if len(ins1) > 0:
                chunk_modify_in_place_set_to_zero(MITprofs['prof_Tweight'], ins1, chunk_size)
        ...
        if zero_criteria_code_i == 3: #  missing T or S
            ...
            #MITprofs['prof_T'][ins3] = fillVal
            #MITprofs['prof_Tweight'][ins3] = 0
            #zero_T_weight_reason[ins3] = zero_T_weight_reason[ins3] + 2**(zero_criteria_code_i - 1)
            if len(ins3) > 0:
                chunk_modify_in_place_add_scalar(MITprofs['prof_T'], ins3, fillVal, chunk_size)
                chunk_modify_in_place_set_to_zero(MITprofs['prof_Tweight'], ins3, chunk_size)
                chunk_modify_in_place_add_scalar(zero_T_weight_reason, ins3, 2**(zero_criteria_code_i - 1), chunk_size)
            ...
            if 'prof_S' in MITprofs:
                ...
                #MITprofs['prof_S'][ins3] = fillVal
                #MITprofs['prof_Sweight'][ins3] = 0
                #zero_S_weight_reason[ins3] = zero_S_weight_reason[ins3] + 2**(zero_criteria_code_i - 1)      
                if len(ins3) > 0:
                    chunk_modify_in_place_add_scalar(MITprofs['prof_S'], ins3, fillVal, chunk_size)
                    chunk_modify_in_place_set_to_zero(MITprofs['prof_Sweight'], ins3, chunk_size)
                    chunk_modify_in_place_add_scalar(zero_S_weight_reason, ins3, 2**(zero_criteria_code_i - 1), chunk_size)
        ...
        if zero_criteria_code_i == 4: # T or S identically zero
            ...
            #MITprofs['prof_T'][ins1] = fillVal
            #MITprofs['prof_Tweight'][ins1] = 0
            #zero_T_weight_reason[ins1] = zero_T_weight_reason[ins1] + 2**(zero_criteria_code_i - 1)
            if len(ins1) > 0:
                chunk_modify_in_place_add_scalar(MITprofs['prof_T'], ins1, fillVal, chunk_size)
                chunk_modify_in_place_set_to_zero(MITprofs['prof_Tweight'], ins1, chunk_size)
                chunk_modify_in_place_add_scalar(zero_T_weight_reason, ins1, 2**(zero_criteria_code_i - 1), chunk_size)
            ...
            if 'prof_S' in MITprofs:
                ...
                #MITprofs['prof_S'][ins1] = fillVal
                #MITprofs['prof_Sweight'][ins1] = 0
                #zero_S_weight_reason[ins1] = zero_S_weight_reason[ins1] + 2**(zero_criteria_code_i -1)
                if len(ins1) > 0:
                    chunk_modify_in_place_add_scalar(MITprofs['prof_S'], ins1, fillVal, chunk_size)
                    chunk_modify_in_place_set_to_zero(MITprofs['prof_Sweight'], ins1, chunk_size)
                    chunk_modify_in_place_add_scalar(zero_S_weight_reason, ins1, 2**(zero_criteria_code_i - 1), chunk_size)
        ...
        if zero_criteria_code_i == 7: # illegal dates/times
            ...
            bad_profs = np.unravel_index(bad_profs, MITprofs['prof_Tweight'].shape, order = 'F')

            #MITprofs['prof_Tweight'][bad_profs, :] = 0
            #zero_T_weight_reason[bad_profs] = zero_T_weight_reason[bad_profs] + 2**(zero_criteria_code_i -1)
            if len(bad_profs) > 0:
                chunk_modify_in_place_set_to_zero(MITprofs['prof_Tweight'], bad_profs, chunk_size)
            ...
            if 'prof_S' in MITprofs:
                ...
                if len(bad_profs) > 0:
                    chunk_modify_in_place_set_to_zero(MITprofs['prof_Sweight'], bad_profs, chunk_size)
        ...
        if zero_criteria_code_i == 9 or zero_criteria_code_i == 10: # high cost vs. climatology
            ...
            if exclude_high_latitude_profiles_from_clim_cost:
                ...
                high_lat_profs_ins_unraveled = np.unravel_index(high_lat_profs_ins, MITprofs['prof_Tweight'].shape, order = 'F')
            ...
            if zero_criteria_code_i == 9: # CHECK AVERAGE COST VS CLIM
                ...
                if 'prof_S' in MITprofs:
                    ...
                    bad_profs = np.unravel_index(bad_profs, MITprofs['prof_Sweight'].shape, order = 'F')
            ...
            if zero_criteria_code_i == 10: # individual points exceed cost threshold
                ...
                bad_profs = np.where(T_cost_vs_clim.flatten(order = 'F') >= single_datum_cost_threshold)
                ...
                if 'prof_S' in MITprofs:
                    ...
                    bad_profs = np.where(S_cost_vs_clim.flatten(order = 'F') >= single_datum_cost_threshold)
        ...
        if zero_criteria_code_i == 11: # test for possible bad conducivity cell.
            if 'prof_S' in MITprofs:
                ...
                for ii in np.arange(num_tests):
                    ...
                    ssdi = np.where(np.abs(MITprofs['prof_depth']) <= np.abs(prof_S_sstd))
                    ...
                    tmp = np.full(MITprofs['prof_S'].shape, np.nan)

                    #tmp[ins1] = 1
                    #tmp[ins2] = 0
                    '''
                    ins2 = np.unravel_index(ins2, tmp.shape, order = 'F')
                    ins1 = np.unravel_index(ins1, tmp.shape, order = 'F')
                   '''
                    if len(ins1) > 0:
        ...
        zero_T_weight_reason_da = xr.DataArray(zero_T_weight_reason, dims=['iPROF','iWEIGHT'], name='zero_T_wight_reason')
        MITprofs['prof_Tweight_code'] = zero_T_weight_reason_da
            
        if 'prof_S' in MITprofs:
            num_nans = np.where(np.isnan(zero_S_weight_reason))[0]
            if len(num_nans) > 0:
                raise Exception('nans found in prof s weight code')
            zero_S_weight_reason_da = xr.DataArray(zero_S_weight_reason, dims=['iPROF','iWEIGHT'], name='zero_S_wight_reason')
            MITprofs['prof_Sweight_code'] = zero_S_weight_reason_da

    #MITprofs.update(MITprofs)
