...
def pp(zarr_file, variable_key, depth_key):
    ...
    # Macro patches
    patch_polygon_list = []
    for patch_dex in range(len(patch_collection_pieces_dict['macro']['polygon_vertex_list'])): 
        patch_list.append(patches.Polygon(patch_collection_pieces_dict['macro']['polygon_vertex_list'][patch_dex], closed=True))

    patch_collection_macro = PatchCollection(patch_list, transform=ccrs.PlateCarree(), joinstyle='miter')

    #patch_collection_pieces_dict['micro']['face_value'] = patch_face_value_list
    ...
    patch_collection_macro.set_array(np.array(anomaly_var_face_list))
    patch_collection_macro.set_linewidths(original_linewidths_plot)
    patch_collection_macro.set_edge_colors(edgecolors=edgecolors_list)
    ...
    patch_collection_micro = PatchCollection(micro_patch_collection_pieces_list, transform=ccrs.PlateCarree(), joinstyle='miter')
    patch_collection_micro.set_array(np.array(micro_patch_collection_pieces_anomaly_list))
    patch_collection_micro.set_linewidths(micro_patch_collection_pieces_linewidths_list)
    patch_collection_micro.set_edge_colors(edgecolors=micro_patch_collection_pieces_edgecolors_list)
