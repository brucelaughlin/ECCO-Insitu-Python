...
def pp(zarr_file, variable_key, depth_key):

    # This parameter determines when we switch to plotting a patch for each profile.  It's the ratio of the current zoom to the original zoom
    #scale_threshold = 20
    scale_threshold = 10
    ...
    face_value_list = patch_collection_pieces_dict['micro']['face_value_list']
    ...
    cbar_min, cbar_max = find_colorbar_limits(cbar, face_value_list)
    ...
    quartiles_edgecolors = np.quantile(np.array(patch_collection_pieces_dict['macro']['edgecolors_list']), quartiles)
    ...
    if scale_factor > scale_threshold: 
        ...
        #collection, new_face_value_list = determine_sub_polygons(ax, patch_list, anomaly_var_raw_values_list, cmap_face, norm_face, edgecolors_list, num_subpolygons_max)
        ...
        if patch_collection_macro != 0:
            ...
            #cbar_min, cbar_max = find_colorbar_limits(cbar, new_face_value_list)
            cbar.ax.set_ylim(cbar_min, cbar_max)
...
def scale_with_zoom(axes, num_artists_original, colorbar, globe_area, linewidth_floor, count_array, patch_collection_pieces_dict, patch_collection_macro, patch_collection_micro, face_value_list, norm_face, scale_threshold, num_subpolygons_max, patch_list_macro, patch_list_micro):

#def scale_with_zoom(axes, num_artists_original, colorbar, globe_area, linewidth_floor, count_array, patch_collection_pieces_dict, patch_collection_macro, patch_collection_micro, face_value_list, norm_face, scale_threshold, num_subpolygons_max):
    ...
    if scale_factor < scale_threshold:
        ...
        cbar_min, cbar_max = find_colorbar_limits(colorbar, face_value_list)
        colorbar.ax.set_ylim(cbar_min, cbar_max)

    else:
        #collection, new_face_value_list = determine_sub_polygons(axes, patch_list, anomaly_var_raw_values_list, cmap_face, norm_face, edgecolors_list, num_subpolygons_max)
